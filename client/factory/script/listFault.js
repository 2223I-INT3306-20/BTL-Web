    ...
    xhttp.onreadystatechange = function(){
        if(this.readyState==4){
            if(this.status==200){
                ...
                document.querySelector("#faultCant tbody").innerHTML = "";
                ...
                for(let i = 0; i< objArr.length; i++){
                    ...
                    document.querySelector("#faultCant tbody").appendChild(tr);
                }
            }
        }
    }
    xhttp.open("GET","http://localhost:8080/factory/cantWarranty",true);
...
window.addEventListener('DOMContentLoaded', event => {
    const datatablesSimple = document.getElementById('faultCant');
    if (datatablesSimple) {
